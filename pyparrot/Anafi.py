...
class Anafi:
    def __init__(self, drone_type=Model.ANAFI, ip_address=None):
        """
    ...
    def update_sensors(self, data_type, buffer_id, sequence_number, raw_data, ack):
        ...
        if sensor_list is not None:
            for sensor in sensor_list:
                (sensor_name, sensor_value, sensor_enum, _header_tuple) = sensor
                if sensor_name is not None:
                    self.sensors.update(sensor_name, sensor_value, sensor_enum)
                else:
                    color_print(
                        "data type %d buffer id %d sequence number %d" % (data_type, buffer_id, sequence_number), "WARN"
                    )
    ...
    def safe_takeoff(self, timeout):
        ...
        while self.sensors.flying_state not in ("flying", "hovering") and (time.time() - start_time < timeout):
            if self.sensors.flying_state == "emergency":
                return
            self.smart_sleep(1)
    ...
    def set_video_stream_mode(self, mode="low_latency"):
        ...
        if fixed_mode not in ("low_latency", "high_reliability", "high_reliability_low_framerate"):
            print(
                "Error: {} is not a valid stream mode.  Must be one of {}".format(
                    mode, "low_latency, high_reliability or high_reliability_low_framerate"
                )
            )
            print("Ignoring command and returning")
            return False
    ...
    def pan_tilt_camera(self, tilt_degrees, pan_degrees):
        ...
        if self.drone_type is Model.ANAFI:
            ...
            self.drone_connection.send_param_command_packet(
                command_tuple, param_tuple=[tilt_degrees, pan_degrees], param_type_tuple=["float", "float"], ack=False
            )
        else:
            command_tuple = self.command_parser.get_command_tuple("ardrone3", "Camera", "Orientation")

            self.drone_connection.send_param_command_packet(
                command_tuple, param_tuple=[tilt_degrees, pan_degrees], param_type_tuple=["i8", "i8"], ack=False
            )
    ...
    def pan_tilt_camera_velocity(self, tilt_velocity, pan_velocity, duration=0):
        ...
        self.drone_connection.send_param_command_packet(
            command_tuple, param_tuple=[tilt_velocity, pan_velocity], param_type_tuple=["float", "float"], ack=False
        )
        ...
        if duration > 0:
            ...
            self.drone_connection.send_param_command_packet(
                command_tuple, param_tuple=[0, 0], param_type_tuple=["float", "float"], ack=False
            )
    ...
    def set_white_balance(self, type):
        ...
        if type not in ("auto", "tungsten", "daylight", "cloudy", "cool_white"):
            print(
                "Error: %s is not valid value. The value must be : auto, tungsten, daylight, cloudy, cool_white" % type
            )
            print("Ignoring command and returning")
            return
    ...
    def set_timelapse(self, enable, interval=8):
        ...
        if enable not in (0, 1) or interval < 8 or interval > 300:
            print(f"Error: {enable} or {interval} is not valid value.")
            print("Ignoring command and returning")
            return

        command_tuple = self.command_parser.get_command_tuple("ardrone3", "PictureSettings", "TimelapseSelection")
        self.drone_connection.send_param_command_packet(
            command_tuple, param_tuple=[enable, interval], param_type_tuple=["u8", "float"]
        )
