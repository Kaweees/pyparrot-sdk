...
class WifiConnection:
    def __init__(self, drone, drone_type: Model, ip_address=None):
        ...
        self.data_types_by_name = {"ACK": 1, "DATA_NO_ACK": 2, "LOW_LATENCY_DATA": 3, "DATA_WITH_ACK": 4}

        # map of the incoming data types by number (to figure out if we need to ack etc)
        self.data_types_by_number = {1: "ACK", 2: "DATA_NO_ACK", 3: "LOW_LATENCY_DATA", 4: "DATA_WITH_ACK"}
    ...
    def connect(self, num_retries):
        ...
        if (self.ip_address is None) and (self.drone_type is not Model.MAMBO):
            ...
            if not self.is_connected:
                color_print(
                    "connection failed: did you remember to connect your machine to the Drone's wifi network?", "ERROR"
                )
                return False
            else:
                browser.cancel()
    ...
    def _listen_socket(self):
        ...
        while self.is_listening:
            try:
                (data, _address) = self.udp_receive_sock.recvfrom(66000)
    ...
    def _send_pong(self, data):
        ...
        packet = struct.pack(
            ...
            self.buffer_ids["PONG"],
            self.sequence_counter["PONG"],
            size + 7,
        )
        packet += data
    ...
    def _handshake(self, num_retries):
        ...
        if self.drone_type in (Model.ANAFI, Model.BEBOP, Model.BEBOP2, Model.DISCO):
            ...
            json_string = json.dumps(
                {
                    "d2c_port": self.udp_receive_port,
            ...
            json_string = json.dumps(
                {"d2c_port": self.udp_receive_port, "controller_type": "computer", "controller_name": "pyparrot"}
            )
    ...
    def send_noparam_high_priority_command_packet(self, command_tuple):
        ...
        packet = struct.pack(
            "<BBBIBBH",
            self.data_types_by_name["LOW_LATENCY_DATA"],
            self.buffer_ids["SEND_HIGH_PRIORITY"],
            self.sequence_counter["SEND_HIGH_PRIORITY"],
            11,
            command_tuple[0],
            command_tuple[1],
            command_tuple[2],
    ...
    def send_param_command_packet(self, command_tuple, param_tuple=None, param_type_tuple=0, ack=True):
        ...
        packet = struct.pack(
            "<BBBIBBH",
            self.data_types_by_name[data_ack_string],
            self.buffer_ids[ack_string],
            self.sequence_counter[ack_string],
            packet_size,
            command_tuple[0],
            command_tuple[1],
            command_tuple[2],
    ...
    def ack_packet(self, buffer_id, packet_id):
        ...
        packet = struct.pack(
            "<BBBIB", self.data_types_by_name["ACK"], new_buf_id, self.sequence_counter[new_buf_id], 8, packet_id
        )
